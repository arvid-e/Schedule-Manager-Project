...
describe("EventRepository", () => {
  ...
  describe("createEvent", () => {
    type MockDocumentContext = IEventDataDocument & {
      toObject: () => IEventDataDocument;
    };
  ...
  describe("updateEvent", () => {
    ...
    it("should return false if no event was edited", async () => {
      ...
      expect(mockEventModel.updateOne).toHaveBeenCalledTimes(1);
      expect(mockEventModel.updateOne).toHaveBeenCalledWith(
        { _id: notFoundMockUpdateData._id },
        { $set: expectedNotFoundUpdateFields },
      );
    });
  });
});
