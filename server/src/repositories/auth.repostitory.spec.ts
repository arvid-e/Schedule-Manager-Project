...
describe("AuthRepository", () => {
  ...
  describe("findUser", () => {
    it("should return user", async () => {
      ...
      const findOneResultFromMethod =
        mockUserModel.findOne.mock.results[1]?.value;
  ...
  describe("createUser", () => {
    it("should create a new user successfully", async () => {
      ...
      (mockUserModel.create as Mock).mockImplementationOnce(
    ...
    it("should throw ConflictError if username already exists", async () => {
      ...
      await expect(authRepository.createUser(createUserData)).rejects.toThrow(
    ...
    it("should throw DatabaseError for validation failure", async () => {
      ...
      mockValidationError.message =
        "Validation failed: Path `password` (`bad`) is shorter than the minimum allowed length (8).";
      mockValidationError.errors = { password: { message: "..." } as any };
      (mockUserModel.create as Mock).mockRejectedValueOnce(mockValidationError);

      await expect(authRepository.createUser(createUserData)).rejects.toThrow(
        "Validation failed: Validation failed: Path `password` (`bad`) is shorter than the minimum allowed length (8).",
      );
      expect(mockUserModel.create).toHaveBeenCalledTimes(1);
      expect(mockUserModel.create).toHaveBeenCalledWith(createUserData);
    });
    ...
    it("should throw generic DatabaseError for other failures", async () => {
      ...
      await expect(authRepository.createUser(createUserData)).rejects.toThrow(
        "Failed to create user: Network issue",
      );
      expect(mockUserModel.create).toHaveBeenCalledTimes(1);
      expect(mockUserModel.create).toHaveBeenCalledWith(createUserData);
    });
