...
describe("EventService", () => {
  ...
  describe("getAllEvents", () => {
    const mockEvents: IEventDataDocument[] = [
      createMockDocument({
        _id: new Types.ObjectId("60d5ec49f1c7f4001c9c7c7c"),
        title: "Concert",
        description: "Desc1",
      }),
      createMockDocument({
        _id: new Types.ObjectId("60d5ec49f1c7f4001c9c7c7d"),
        title: "Meeting",
        description: "Desc2",
      }),
    ];
  ...
  describe("createEvent", () => {
    ...
    it("should propagate errors from the repository", async () => {
      ...
      await expect(eventService.createEvent(createInput)).rejects.toThrow(
        repoError,
      );
      expect(mockEventRepository.createEvent).toHaveBeenCalledTimes(1);
    });
  ...
  describe("deleteEvent", () => {
    ...
    it("should propagate errors from the repository", async () => {
      ...
      await expect(eventService.deleteEvent(eventIdToDelete)).rejects.toThrow(
        repoError,
      );
      expect(mockEventRepository.deleteEvent).toHaveBeenCalledTimes(1);
    });
  ...
  describe("updateEvent", () => {
    ...
    it("should propagate errors from the repository", async () => {
      ...
      await expect(eventService.updateEvent(updateData)).rejects.toThrow(
        repoError,
      );
      expect(mockEventRepository.updateEvent).toHaveBeenCalledTimes(1);
    });
