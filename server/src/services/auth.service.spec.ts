...
vi.mock("bcryptjs", () => ({
  default: {
    // <--- This is the key change: provide a 'default' export
    hash: vi.fn(),
    compare: vi.fn(),
  },
}));
...
describe("AuthService", () => {
  ...
  beforeEach(() => {
    ...
    authService = new AuthService(
      mockAuthRepository as unknown as AuthRespository,
  ...
  describe("loginUser", () => {
    ...
    const mockUserFound: IUser & { password?: string } = {
      // Add password for the mock user from DB
      _id: "user123",
      username: "testuser",
    ...
    it("should successfully log in a user with correct credentials", async () => {
      ...
      expect(mockAuthRepository.findUser).toHaveBeenCalledWith(
      ...
      expect(mockedBcrypt.compare).toHaveBeenCalledWith(
        ...
        mockUserFound.password,
      );

      expect(mockTokenService.generateJwtToken).toHaveBeenCalledTimes(1);
      expect(mockTokenService.generateJwtToken).toHaveBeenCalledWith({
        userId: mockUserFound._id,
      });
    ...
    it("should re-throw DatabaseError from authRepository.findUser", async () => {
      ...
      await expect(authService.loginUser(mockLoginData)).rejects.toThrow(
        "An unexpected database error occurred during login. Please try again later.",
      );
      expect(mockAuthRepository.findUser).toHaveBeenCalledTimes(1);
      expect(mockedBcrypt.compare).not.toHaveBeenCalled();
      expect(mockTokenService.generateJwtToken).not.toHaveBeenCalled();
  ...
  describe("registerUser", () => {
    ...
    it("should successfully register a new user", async () => {
      ...
      expect(mockedBcrypt.hash).toHaveBeenCalledWith(
        ...
        MOCK_BCRYPT_SALT_ROUNDS,
      ); // Use the mock constant
    ...
    it("should throw an error if username is already taken", async () => {
      ...
      await expect(authService.registerUser(mockRegisterData)).rejects.toThrow(
    ...
    it("should throw ConflictError if createUser fails due to conflict", async () => {
      ...
      await expect(authService.registerUser(mockRegisterData)).rejects.toThrow(
        "Registration failed: User already exists in DB",
      );
    ...
    it("should throw DatabaseError if createUser fails due to database issue", async () => {
      ...
      await expect(authService.registerUser(mockRegisterData)).rejects.toThrow(
        "Could not register user due to a database issue.",
      );
    ...
    it("should re-throw generic error if bcrypt.hash fails", async () => {
      ...
      await expect(authService.registerUser(mockRegisterData)).rejects.toThrow(
        "Failed to register user due to an unknown error.",
      );
